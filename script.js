...
// restAreaData 대신 processedTourismData를 사용할 예정
let processedTourismData = null; 
...
// 기존 Firebase 함수를 대체하여 JSON 파일에서 관광 데이터 로드
async function loadTourismDataFromJSON() {
    try {
        showLoadingSpinner('관광 데이터를 불러오는 중...');
        
        // 광역 지자체 데이터 로드
        const metroResponse = await fetch('./data/metro-current.json');
        const metroData = await metroResponse.json();
        
        // 기초 지자체 데이터 로드
        const localResponse = await fetch('./data/local-current.json');
        const localData = await localResponse.json();
        
        // 데이터 처리 및 가공
        processedTourismData = processTourismData(metroData, localData);
        
        hideLoadingSpinner();
        showFloatingMessage(`✅ 관광 데이터 ${processedTourismData.length}개를 성공적으로 로드했습니다!`, 'success', 4000);
        
        return processedTourismData;
        
    } catch (error) {
        hideLoadingSpinner();
        console.error('관광 데이터 로드 실패:', error);
        showFloatingMessage('⚠️ 관광 데이터 로드에 실패했습니다. 새로고침해 주세요.', 'error', 5000);
        return [];
    }
}

// 한국관광공사 TourAPI 응답 구조에 맞게 데이터 처리 및 가공
function processTourismData(metroData, localData) {
    const data = [];
    
    // 광역 지자체 데이터 처리
    if (metroData?.response?.body?.items?.item) {
        const items = Array.isArray(metroData.response.body.items.item) 
            ? metroData.response.body.items.item 
            : [metroData.response.body.items.item];
        ...
        items.forEach(item => {
            data.push({
                ...
                areaNm: item.areaNm, // 시도명 
                touNum: parseFloat(item.touNum) || 0, // 관광객수 
                touDivNm: item.touDivNm, // 관광객구분명 
                baseYmd: item.baseYmd // 기준연월일 
            });
        });
    }
    
    // 기초 지자체 데이터 처리
    if (localData?.response?.body?.items?.item) {
        const items = Array.isArray(localData.response.body.items.item) 
            ? localData.response.body.items.item 
            : [localData.response.body.items.item];
...
function updateMapDisplay() {
    ...
    if (activeThemes.length === 0) {
        ...
        // processedTourismData를 사용할 수 있지만, 휴게소 데이터만 필터링하는 로직이 필요.
        // 현재는 TourAPI에서 제공하는 관광객수 데이터이므로 휴게소 수와는 다름.
        // 따라서 restAreaData 대신 processedTourismData.length를 사용하여 일반적인 관광 데이터 수를 표시.
        if (activeThemes[0] === 'restarea' && processedTourismData) {
            const restAreaCount = processedTourismData.filter(d => d.type === 'restarea').length; // 가상의 휴게소 데이터 필터링
            updateCurrentCategoryDisplay(`${themeName} (${restAreaCount}개) - ${locationText}`);
        } else {
...
// 휴게소 표시 (여기서는 샘플 데이터를 사용하며, 실제 API 데이터에는 휴게소 정보가 직접 포함되지 않을 수 있음)
function showRestAreas() {
    // TourAPI에서는 '휴게소'와 같은 특정 시설의 지리적 좌표를 직접 제공하지 않습니다.
    // 여기서는 개념적인 '휴게소' 마커를 표시하는 예시로, 실제 데이터는 TourAPI의 '관광빅데이터 정보 서비스'에서 제공하는 방문객 수 등입니다. 
    // 따라서 이 함수는 기존 샘플 데이터 로직을 유지하거나, 실제 TourAPI의 다른 엔드포인트를 통해 휴게소 데이터를 얻어야 합니다.
    // 현재는 샘플 데이터를 사용하므로, 실제 API 데이터 로드 시 이 부분은 변경이 필요합니다.
    
    const sampleRestAreas = getSampleRestAreaData(); // 샘플 휴게소 데이터 사용
    
    if (!sampleRestAreas || !Array.isArray(sampleRestAreas) || sampleRestAreas.length === 0) {
        showFloatingMessage('표시할 수 있는 휴게소 샘플 데이터가 없습니다.', 'error', 5000);
        return;
    }
    ...
    map.setView([36.5, 127.5], 7);
    updateCurrentCategoryDisplay(`고속도로 휴게소 (샘플 ${sampleRestAreas.length}개)`); // 샘플 데이터임을 명시

    showLoadingSpinner(`${sampleRestAreas.length}개 휴게소 마커를 생성하는 중...`);

    let successCount = 0;
    let errorCount = 0;
    const processedHighways = new Set();
    
    sampleRestAreas.forEach((restArea) => {
        try {
            const lat = parseFloat(restArea['위도']);
            const lng = parseFloat(restArea['경도']);
            ...
            errorCount++;
            console.error('휴게소 마커 생성 중 오류:', error);
        }
    });
...
// 샘플 휴게소 데이터 생성 (Firestore 연결 실패 시 fallback) - 기존 코드 유지
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // 휴게소 테마가 활성화되어 있고, 데이터가 로드된 경우 (샘플 데이터 사용)
    if (themeStates.restarea && getSampleRestAreaData().length > 0) {
        showRestAreas();
    }
